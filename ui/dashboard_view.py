...
def create_dashboard(parent, layout, t, log_func=None, clear_log_func=None, lang="zh"):
    ...
    # 动态切换表格拖拽使能状态
    cb_disable_drag.stateChanged.connect(
        lambda s: [tabs.widget(i)._set_drag_enabled(s == Qt.Unchecked) for i in range(tabs.count()) if
                   hasattr(tabs.widget(i), "_set_drag_enabled")])
    ...
    def load_initial():
        """加载初始默认配置文件"""
        path = FileService.get_last_workbench_file()
        if not path:
            ...
            FileService.set_last_workbench_file(path)
    ...
    def do_save():
        ...
        data = {tabs.tabText(i): [[tabs.widget(i).item(r, c).text() if tabs.widget(i).item(r, c) else "" for c in
                                   range(tabs.widget(i).columnCount())] for r in range(tabs.widget(i).rowCount())] for i
                in range(tabs.count())}
    ...
    def on_edit(table, row):
        ...
        try:
            ...
            UIGenerator.handle_exception(parent, t, e, log, t('update_step_err'))
    ...
    def on_action(action_func):
        ...
        try:
            ...
            UIGenerator.handle_exception(parent, t, e, log)

    def on_start():
        """启动抓取逻辑：提取当前 Sheet 数据并传递给抓取服务"""
        table = tabs.currentWidget()
        if not table: return log(t("no_active_sheet"), "orange")
        data = [{f["key"]: (table.item(r, c).text() if table.item(r, c) else "") for c, f in enumerate(fields_config)}
    ...
    btn_del_step.clicked.connect(lambda: on_action(lambda tbl: tbl.currentRow() != -1 and (
                tbl.removeRow(tbl.currentRow()) or tbl.on_data_structure_changed() or do_save())))
    btn_add_sheet.clicked.connect(lambda: add_sheet())
    btn_del_sheet.clicked.connect(
        lambda: tabs.currentIndex() != -1 and (tabs.removeTab(tabs.currentIndex()) or do_save()))
